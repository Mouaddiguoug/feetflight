...
class sellerController {
  ...
  public createSubscribePlans = async (
    req: Request,
    res: Response,
    next: NextFunction
  ): Promise<void> => {
    try {
      ...
      const createdSubscriptionPlans = await this.sellerService.createSubscribePlans(
        userId,
  ...
  public requestWithdraw = async (
    ...
    try {
      ...
      res.status(requestedWithdraw ? 201 : 400).json({
        message: requestedWithdraw
          ? 'Your withdraw request is being account has been added successfully!'
    ...
    try {
      ...
      res.status(payoutAccount ? 201 : 400).json({
        message: payoutAccount
          ? 'Your payout account has been added successfully!'
          : 'Something went wrong!',
      });
    } catch (error) {
      next(error);
  ...
  public deletePayoutAccount = async (
    req: Request,
    res: Response,
    next: NextFunction
  ): Promise<void> => {
  ...
  public getPayoutAccounts = async (
    req: Request,
    res: Response,
    next: NextFunction
  ): Promise<void> => {
  ...
  public uploadSentPicture = async (
    req: Request,
    res: Response,
    next: NextFunction
  ): Promise<void> => {
    try {
      ...
      const sentPictureData = await this.sellerService.uploadSentPicture(
        sentPictureDataFile,
        userId,
        tipAmount,
        receiverId
  ...
  public uploadIdentityCard = async (
    req: Request,
    res: Response,
    next: NextFunction
  ): Promise<void> => {
    try {
      ...
      res
        .status(201)
        .json({ message: 'identity card haq been uploaded successfully', status: 200 });
    } catch (error) {
      next(error);
    }
  };
