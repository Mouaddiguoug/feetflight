...
function validateSecrets() {
  ...
  if (!process.env.REFRESH_SECRET) {
    appLogger.warn(
      'REFRESH_SECRET not set, falling back to SECRET_KEY. Consider setting a separate refresh secret for enhanced security.'
    );
  }
}
...
export const authPlugin = () => {
  ...
  return new Elysia({ name: 'plugin.auth', seed: 'plugin.auth' })
    .use(bearer())
      ...
      auth: {
        ...
        createCookie: (tokenData: TokenData): string => {
          ...
          const cookieParts = [
            ...
            'HttpOnly',
            `Max-Age=${tokenData.maxAgeSeconds}`,
            'Path=/',
            'SameSite=Strict',
          ];
...
export const authGuard = () => {
  return new Elysia({ name: 'plugin.auth.guard' })
    .use(bearer())
    .use(neo4jPlugin())
    .use(authPlugin())
    .use(loggerPlugin())
    .resolve(async (ctx) => {
      ...
      if (!accessJwt) {
        const errorMsg =
          'authGuard requires authPlugin() to be registered first. Please use .use(authPlugin()) before .use(authGuard())';
      ...
      try {
        ...
        const userRecord = await neo4j.withSession(async (session: Session) => {
          const result = await session.executeRead((tx) =>
            tx.run('MATCH (u:user {id: $userId}) RETURN u', { userId })
          );
