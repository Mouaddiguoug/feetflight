...
const requestIdPlugin = () => {
  return new Elysia({ name: 'plugin.request-id', seed: 'plugin.request-id' }).onRequest(({ request, set }) => {
...
function createPinoInstance(): Logger {
  ...
  if (isDevelopment) {
    ...
    return pino({
      ...
      redact: {
        paths: ['req.headers.authorization', 'req.headers.cookie', 'password', 'token', 'secret', 'apiKey', 'accessToken', 'refreshToken'],
        censor: '[REDACTED]',
      },
      serializers: {
        req: req => ({
...
export const loggerPlugin = () => {
  ...
  return (
    new Elysia({ name: 'plugin.logger', seed: 'plugin.logger' })
      .use(requestIdPlugin())
