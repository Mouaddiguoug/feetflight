...
export async function getUnverifiedSellers(deps: AdminServiceDeps): Promise<any[]> {
  ...
  try {
    ...
    throw new InternalServerError(`Failed to get unverified sellers: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
...
export async function getSellerIdentityCard(
  ...
  try {
    ...
    throw new InternalServerError(`Failed to get seller identity card: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
