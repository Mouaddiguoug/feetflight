...
export interface AuthServiceDeps {
  ...
  auth: {
    signAccessToken: (userId: string) => Promise<{ token: string; expiresIn: string; maxAgeSeconds: number }>;
    signRefreshToken: (userId: string) => Promise<{ token: string; expiresIn: string; maxAgeSeconds: number }>;
    signEmailToken: (userId: string) => Promise<{ token: string; expiresIn: string; maxAgeSeconds: number }>;
    createCookie: (tokenData: { token: string; expiresIn: string; maxAgeSeconds: number }) => string;
  };
...
export async function signup(userData: any, deps: AuthServiceDeps): Promise<SignupResponse> {
  ...
  try {
    ...
    const existingUser = await neo4j.withSession(async (session: Session) => {
      const result = await session.executeRead(tx => tx.run('MATCH (u:user {email: $email}) RETURN u', { email }));
    ...
    if (role === RolesEnum.SELLER) {
      return await signupSeller(userData, hashedPassword, email, { auth, neo4j, log, stripe, resend });
    } else if (role === RolesEnum.BUYER) {
      return await signupBuyer(userData, hashedPassword, email, { auth, neo4j, log, stripe, resend });
    } else {
    ...
    throw new InternalServerError(`Signup failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
...
export async function sendVerificationEmail(
  ...
  try {
    ...
    throw new InternalServerError(`Email sending failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
...
export async function login(userData: any, deps: AuthServiceDeps): Promise<LoginResponse> {
  ...
  try {
    ...
    const result = await neo4j.withSession(async (session: Session) => {
      ...
      const findUserResult = await session.executeRead(tx => tx.run('MATCH (u:user {email: $email}) RETURN u', { email }));
      ...
      const roleResult = await session.executeRead(tx => tx.run('MATCH (u:user {id: $id})-[:IS_A]-(r:seller) RETURN r', { id: userId }));

      const role = roleResult.records.length === 0 ? 'Buyer' : 'Seller';

      // Update device token (merge to ensure node exists)
      await session.executeWrite(tx =>
        tx.run('MATCH (u:user {id: $id}) MERGE (u)-[:logged_in_with]->(d:deviceToken) SET d.token = $token', {
          id: userId,
          token: deviceToken,
        }),
      );
    ...
    throw new InternalServerError(`Login failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
 ...
 */
export async function changePassword(email: string, userData: any, deps: AuthServiceDeps): Promise<any> {
  ...
  try {
    ...
    throw new InternalServerError(`Change password failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
...
export async function resendVerificationEmail(email: string, deps: AuthServiceDeps): Promise<void> {
  ...
  try {
    const result = await neo4j.withSession(async (session: Session) => {
      // Find user
      const userResult = await session.executeRead(tx => tx.run('MATCH (u:user {email: $email}) RETURN u', { email }));
    ...
    // Send verification email
    await sendVerificationEmail(email, result.userProperties.userName, tokenData.token, result.role, resend);
    ...
    throw new InternalServerError(`Resend verification email failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
 ...
 */
export async function refreshToken(id: string, deps: AuthServiceDeps): Promise<{ tokenData: TokenData }> {
  const { auth, log } = deps;
  ...
  try {
    ...
    throw new InternalServerError(`Refresh token generation failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
...
export async function logout(user: any, deps: AuthServiceDeps): Promise<any> {
  ...
  await deps.neo4j?.withSession(async (session) => {
    await session.executeWrite(tx =>
      tx.run('MATCH (u:user {id: $id})-[:logged_in_with]->(d:deviceToken) SET d.token = ""', { id: user.id })
    );
  });
