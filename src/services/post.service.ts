...
export async function getPopularAlbums(userId: string, deps: PostServiceDeps): Promise<any[]> {
  ...
  try {
    ...
    throw new InternalServerError(`Failed to get popular albums: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}


export async function getRandomAlbums(page: number, userId: string, deps: PostServiceDeps): Promise<any[]> {
  const { neo4j, log } = deps;
  ...
  try {
    ...
    throw new InternalServerError(`Failed to get random albums: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}


export async function getAlbumByCategory(categoryId: string, deps: PostServiceDeps): Promise<any[]> {
  const { neo4j, log } = deps;
  ...
  try {
    ...
    throw new InternalServerError(`Failed to get album by category: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}


export async function deleteAlbum(albumId: string, deps: PostServiceDeps): Promise<{ message: string }> {
  const { neo4j, log } = deps;
  ...
  try {
    ...
    throw new InternalServerError(`Failed to delete album: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
...
export async function getCategories(deps: PostServiceDeps): Promise<any[]> {
  ...
  try {
    ...
    throw new InternalServerError(`Failed to get categories: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
...
export async function getAlbumPlan(albumId: string, deps: PostServiceDeps): Promise<any> {
  ...
  try {
    ...
    throw new InternalServerError(`Failed to get album plan: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
...
export async function getAllAlbums(userId: string, deps: PostServiceDeps): Promise<any[]> {
  ...
  try {
    ...
    throw new InternalServerError(`Failed to get all albums: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
...
export async function getSellerAlbums(userId: string, deps: PostServiceDeps): Promise<any[]> {
  ...
  try {
    ...
    throw new InternalServerError(`Failed to get seller albums: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
...
export async function getPostPictures(postId: string, deps: PostServiceDeps): Promise<any[]> {
  ...
  try {
    ...
    throw new InternalServerError(`Failed to get post pictures: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
...
export async function updateViews(postId: string, deps: PostServiceDeps): Promise<number> {
  ...
  try {
    ...
    throw new InternalServerError(`Failed to update views: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}


export async function createPost(userId: string, postData: any, deps: PostServiceDeps): Promise<{ post: any; collection: any }> {
  ...
  try {
    ...
    const result = await neo4j.withSession(async (session: Session) => {
      // Find user
      const findUser = await session.executeRead(tx => tx.run('match (u:user {id: $userId}) return u', { userId: userId }));
    ...
    log?.info({ userId, postId: result.records[0]?.get('p').properties.id }, 'Post created successfully');
    ...
    if (error instanceof NotFoundError || error instanceof BadRequestError || error instanceof InternalServerError) {
      throw error;
    }

    throw new InternalServerError(`Failed to create post: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}


export async function likePost(albumId: string, userId: string, deps: PostServiceDeps): Promise<void> {
  const { neo4j, log } = deps;
  ...
  try {
    ...
    throw new InternalServerError(`Failed to like post: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}

export async function uploadPostPictures(pictureFiles: Array<{ buffer: Buffer; mimetype: string; fieldname: string }>, collectionId: string, deps: PostServiceDeps): Promise<void> {
  const { log } = deps;
  ...
  try {
    ...
    await Promise.all(writtenFiles.map(file => createPictures(file.fieldname, file.path, collectionId, deps)));

    log?.info({ collectionId, fileCount: writtenFiles.length }, 'Post pictures uploaded successfully');
  } catch (error) {
    log?.error({ error, collectionId }, 'Upload post pictures failed');
    throw new InternalServerError(`Failed to upload post pictures: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}


export async function createPictures(pictureDescription: string, value: string, collectionId: string, deps: PostServiceDeps): Promise<void> {
  const { neo4j, log } = deps;
  ...
  try {
    ...
    throw new InternalServerError(`Failed to create picture: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
...
export default class postService {
   ...
   */
  public async uploadPostPictures(pictureFiles: Array<{ buffer: Buffer; mimetype: string; fieldname: string }>, collectionId: string): Promise<any> {
    throw new Error(
      'postService class is deprecated. This method is non-functional. ' +
        'Use the Elysia routes in src/routes/post.route.ts instead.'
    );
   ...
   */
  public async createPictures(pictureDescription: string, value: string, collectionId: string): Promise<any> {
    throw new Error(
      'postService class is deprecated. This method is non-functional. ' +
        'Use the Elysia routes in src/routes/post.route.ts instead.'
    );
