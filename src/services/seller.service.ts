...
export async function createSubscribePlans(
  ...
  try {
    ...
    const createdSubscriptionPlans = subscriptionPlansData.data.subscriptionPlans.map((subscriptionPlan: any) => {
      return createSubscribePlan(subscriptionPlan.subscriptionPlanPrice, subscriptionPlan.subscriptionPlanTitle, userId, deps);
    });
...
export async function changePlans(plans: any[], deps: SellerServiceDeps): Promise<{ message: string }> {
  try {
    if (!deps.stripe) {
      throw new InternalServerError('Stripe is not configured');
    }
  ...
  try {
    const updatedPlan = await deps.neo4j.withSession(async session => {
      return await session.executeWrite(tx =>
        tx.run('match (subscriptionPlan:subscriptionPlan {id: $planId}) set subscriptionPlan.id = $newPlanId, subscriptionPlan.title = $name, subscriptionPlan.price = $price', {
          planId: oldPlanId,
          newPlanId: newPlanId,
          name: name,
          price: price,
...
export async function getSubscriptionPlans(userId: string, deps: SellerServiceDeps): Promise<any[]> {
  try {
...
export async function addPayoutAccount(userId: string, bankAccountData: any, deps: SellerServiceDeps): Promise<void> {
  try {
...
export async function requestWithdraw(userId: string, payoutAccountId: string, deps: SellerServiceDeps): Promise<void> {
  try {
...
export async function getFollowersCount(sellerId: string, deps: SellerServiceDeps): Promise<number> {
  try {
...
export async function uploadIdentityCard(identityCardData: any, userId: string, deps: SellerServiceDeps): Promise<void> {
  try {
  ...
  try {
    await deps.neo4j.withSession(async session => {
      switch (side) {
        ...
        case 'backSide':
          return await session.executeWrite(tx =>
            tx.run('match (user {id: $userId})-[:IS_A]->(s:seller) set s.backIdentityCard = $backIdentityCard', {
              userId: userId,
...
class sellerService {
  ...
  public async getAllSellers() {
    throw new Error(
      'This method is deprecated. Use Elysia seller routes at GET /sellers instead.',
    );
  }
